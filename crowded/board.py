	...
	def build(self):
		...
		for i in range(self.size):
			for j in range(self.size):
				...
				self.grid[i][j].row = i
				self.grid[i][j].col = j
				self.grid[i][j].x = self.grid[i][j].col * SQUARESIZE
				self.grid[i][j].y = self.grid[i][j].row * SQUARESIZE
		...
		for i in range(self.size):
			for j in range(self.size):
				...
				# Esquina inferior izquierda
					...
					self.grid[i][j].vecindad = [(self.grid[i-1][j]), (self.grid[i][j+1]),(self.grid[i-1][j+1]),]
